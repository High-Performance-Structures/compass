...
export function MessageCard({ projectId, message, viewerId, editDisabled, onEdit, onRetract }: {
  ...
  return <article id={`correspondence-message-${message.id}`} className="border p-4 scroll-mt-48">
    ...
    {message.attachments.length > 0 && <ul className="mt-3 grid gap-2">{message.attachments.map((attachment) => <CorrespondenceAttachmentRow key={attachment.id} projectId={projectId} attachment={attachment} />)}</ul>}
