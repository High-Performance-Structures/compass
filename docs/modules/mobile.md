...
The default release profile loads the live Cloudflare application by setting `server.url`. Run `bun cap:sync` for normal TestFlight and Play builds. This keeps the native release compatible with the deployed web application even while Field Mode is under development.

`mobile-shell/` is generated by `bun run mobile:build` from `mobile-shell-src/`. It contains only the offline field workspace, not a static export of the server-rendered application. Run `bun run cap:sync:field` only when the matching Field Mode API routes have been deployed. In that explicit profile there is no `server.url`, so the shell can cold-start with no network. It reads cached active projects, tasks, schedule items, daily logs, construction documents, project chat, and queued mutations from Capacitor Preferences and Filesystem. When the Compass health endpoint is reachable, it navigates the same WebView to the live Field Mode.
...
The generated `mobile-shell/` directory is ignored by Git. Run `bun cap:sync` before opening Xcode or Android Studio for a normal release. Use `bun run cap:sync:field` only for coordinated Field Mode testing.
