...
function createSchema(sqlite: Sqlite): void {
  sqlite.exec(`
    ...
    CREATE TABLE nutech_products (
      ...
      minimum_order_increment INTEGER NOT NULL,
    ...
    CREATE TABLE nutech_order_workflows (
      ...
      airlite_workbook_claim_token TEXT,
      airlite_workbook_claim_revision INTEGER,
      airlite_workbook_claim_attempt INTEGER,
      airlite_workbook_claim_reclaim_after TEXT,
...
function seedDraft(sqlite: Sqlite, now: string): void {
  ...
  sqlite
    .prepare("UPDATE projects SET google_drive_folder_id = ? WHERE id = ?")
    .run("folder-1", "project-1")
  sqlite.prepare(`
...
function seedNuTechWorkflow(sqlite: Sqlite, now: string): void {
  sqlite.prepare(`
    INSERT INTO nutech_catalog_versions (
      id, organization_id, status, effective_date, airlite_template_id
    ) VALUES (?, ?, ?, ?, ?)
  `).run("catalog-1", "org-1", "active", "2026-01-01", "template-1")
  sqlite.prepare(`
    INSERT INTO nutech_products (
      id, organization_id, manufacturer_sku, name, category, origin,
      price_unit, minimum_order_increment, package_label, airlite_template_row, active
    ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `).run("product-1", "org-1", "SKU-1", "Block", "block", "USA", "each", 1, "Each", 10, 1)
  sqlite.prepare(`
    INSERT INTO nutech_catalog_prices (
      id, catalog_version_id, product_id, airlite_cost_cents,
      new_standard_price_cents, new_cash_price_cents,
      returning_standard_price_cents, returning_cash_price_cents
    ) VALUES (?, ?, ?, ?, ?, ?, ?, ?)
  `).run("price-1", "catalog-1", "product-1", 100, 120, 110, 115, 105)
  sqlite.prepare(`
    INSERT INTO nutech_order_workflows (
      ...
      delivery_method, airlite_purchase_order_operation_id, catalog_version_id, order_status,
...
describe("Nu-Tech purchase-order release versus supplier email", () => {
  ...
  it("does not release a vendor invoice from a stale workflow snapshot", async () => {
    ...
    const staleActor = drizzle(staleClient, {
      schema: { organizations, projects, nuTechOrderWorkflows },
    })
    // @ts-expect-error The SQLite adapter implements the D1 methods exercised here.
    ...
    const staleRelease = releaseNuTechVendorInvoice("project-1")
    await readCompleted
    await currentActor
      .update(nuTechOrderWorkflows)
      .set({
        vendorInvoiceNumber: "INV-CURRENT",
    ...
    releaseRead()
    await expect(staleRelease).resolves.toEqual({
      success: false,
      error:
    ...
    expect(sqlite.prepare(`
      SELECT vendor_invoice_number, vendor_invoice_status,
             vendor_invoice_released_at, order_status
      FROM nutech_order_workflows WHERE id = 'workflow-1'
    `).get()).toEqual({
      vendor_invoice_number: "INV-CURRENT",
      vendor_invoice_status: "received",
...
describe("Nu-Tech Airlite workbook generation claim", () => {
  ...
  it.each([
    { label: "existing", seedExisting: true },
    { label: "new", seedExisting: false },
  ])("does not let a stale $label item save commit after a workbook claim wins", async ({
    seedExisting,
  }) => {
    let releaseBatch: () => void = () => undefined
    const batchPaused = new Promise<void>((resolve) => {
    ...
    if (!seedExisting) {
      sqlite
        .prepare("DELETE FROM nutech_order_items WHERE id = 'nutech-item-1'")
        .run()
    }
    ...
    const staleSave = saveNuTechOrderItem("project-1", {
      productId: "product-1",
      quantity: 2,
    })
    await expect(
    ...
    await expect(staleSave).resolves.toEqual({
      success: false,
      error: "The Nu-Tech order changed while the item was being saved. Refresh and try again.",
    })
    expect(sqlite.prepare(`
      SELECT quantity FROM nutech_order_items WHERE workflow_id = 'workflow-1'
        AND product_id = 'product-1'
    `).get()).toEqual(seedExisting ? { quantity: 1 } : undefined)
    expect(sqlite.prepare(`
      SELECT airlite_workbook_status, airlite_workbook_claim_token,
  ...
  it("does not let a stale item delete commit after a workbook claim wins", async () => {
    let releaseBatch: () => void = () => undefined
    const batchPaused = new Promise<void>((resolve) => {
      releaseBatch = resolve
    })
    ...
    const staleDelete = deleteNuTechOrderItem("project-1", "nutech-item-1")
    await expect(
      Promise.race([
        batchReached.then(() => "batch"),
        staleDelete.then((result) => `completed: ${JSON.stringify(result)}`),
      ])
    ).resolves.toBe("batch")
    await currentActor
      .update(nuTechOrderWorkflows)
    ...
    releaseBatch()
    await expect(staleDelete).resolves.toEqual({
      success: false,
      error: "The Nu-Tech order changed while the item was being deleted. Refresh and try again.",
    })
    expect(sqlite.prepare(`
      SELECT COUNT(*) AS count FROM nutech_order_items WHERE id = 'nutech-item-1'
    `).get()).toEqual({ count: 1 })
  ...
  it("permits one provider workbook copy when two database actors race", async () => {
    ...
    seedNuTechWorkflow(sqlite, FIXED_NOW)
    // @ts-expect-error The SQLite adapter implements the D1 methods exercised here.
    const actorOne = drizzle(createD1(sqlite), {
      schema: {
        organizations,
        projects,
        projectOperations,
    ...
    // @ts-expect-error The SQLite adapter implements the D1 methods exercised here.
    const actorTwo = drizzle(createD1(sqlite), {
      schema: {
        organizations,
        projects,
        projectOperations,
    ...
    const copyFile = vi.fn(
      async (): Promise<{ readonly id: string; readonly webViewLink: string }> => {
        providerStarted()
        if (copyFile.mock.calls.length > 1) {
          return {
            id: "workbook-2",
            webViewLink: "https://drive.test/workbook-2",
          }
        }
    ...
    const addSheet = vi.fn(async () => ({ sheetId: 1 }))
    const listFiles = vi.fn(async () => ({ files: [] }))
    mocks.getOrganizationDriveContext.mockResolvedValue({
      client: { copyFile, listFiles },
      sheetsClient: { batchUpdateValues, addSheet },
      userEmail: "staff@example.com",
    })
    mocks.getDb.mockImplementationOnce(() => actorOne).mockImplementationOnce(() => actorTwo)

    const firstWorker = generateNuTechAirliteWorkbook("project-1")
    await expect(
      Promise.race([
        providerReached.then(() => "provider"),
        firstWorker.then(
          () => "completed",
    ...
    const secondResult = await generateNuTechAirliteWorkbook("project-1")
    ...
    releaseProvider({ id: "workbook-1", webViewLink: "https://drive.test/workbook-1" })
    ...
    const stored = sqlite
      .prepare(
        "SELECT airlite_workbook_status, airlite_workbook_claim_token FROM nutech_order_workflows WHERE id = ?"
      )
      .get("workflow-1") as {
      airlite_workbook_status: string
      airlite_workbook_claim_token: string | null
    }
    expect(stored.airlite_workbook_status).toBe("generated")
    expect(stored.airlite_workbook_claim_token).toBeNull()
    sqlite.close()
  })

  it("holds a durable claim fence across each Sheets provider write", async () => {
    const sqlite = new Database(":memory:")
    createSchema(sqlite)
    seedDraft(sqlite, FIXED_NOW)
    seedNuTechWorkflow(sqlite, FIXED_NOW)
    ...
    let releaseSheets: () => void = () => undefined
    const sheetsPaused = new Promise<void>((resolve) => {
      releaseSheets = resolve
    })
    let signalSheets: () => void = () => undefined
    const sheetsReached = new Promise<void>((resolve) => {
      signalSheets = resolve
    })
    const batchUpdateValues = vi.fn(async () => {
      if (batchUpdateValues.mock.calls.length !== 1) return
      signalSheets()
      await sheetsPaused
    })
    const workbook = {
  ...
  it("reuses a provider artifact with the deterministic generation fingerprint", async () => {
    const sqlite = new Database(":memory:")
    createSchema(sqlite)
    seedDraft(sqlite, FIXED_NOW)
    seedNuTechWorkflow(sqlite, FIXED_NOW)
    ...
    mocks.getDb.mockImplementation(() => actor)

    await expect(generateNuTechAirliteWorkbook("project-1")).resolves.toEqual({
      success: true,
      id: "workflow-1",
      workbookUrl: "https://drive.test/recovered-workbook",
    })
    expect(copyFile).not.toHaveBeenCalled()
    expect(batchUpdateValues).toHaveBeenCalledTimes(1)
    sqlite.close()
  ...
  it("reuses an addendum sheet after an ambiguous prior provider write", async () => {
    const sqlite = new Database(":memory:")
    createSchema(sqlite)
    seedDraft(sqlite, FIXED_NOW)
    seedNuTechWorkflow(sqlite, FIXED_NOW)
    sqlite
      .prepare("UPDATE nutech_products SET airlite_template_row = NULL WHERE id = ?")
      .run("product-1")
    // @ts-expect-error The SQLite adapter implements the D1 methods exercised here.
    const actor = drizzle(createD1(sqlite), {
      schema: {
    ...
    expect(getSpreadsheetMetadata).toHaveBeenCalledTimes(1)
    expect(addSheet).not.toHaveBeenCalled()
    expect(batchUpdateValues).toHaveBeenCalledTimes(2)
    sqlite.close()
  })

  it("preserves an unresolved durable provider attempt across process restart", async () => {
    const sqlite = new Database(":memory:")
    createSchema(sqlite)
    seedDraft(sqlite, FIXED_NOW)
    seedNuTechWorkflow(sqlite, FIXED_NOW)
    ...
    expect(sqlite.prepare(`
      SELECT airlite_workbook_status, airlite_workbook_claim_token,
             airlite_workbook_claim_revision, airlite_workbook_provider_status,
  ...
  it("fences the expired worker when a replacement claim takes over", async () => {
    const sqlite = new Database(":memory:")
    createSchema(sqlite)
    seedDraft(sqlite, FIXED_NOW)
    seedNuTechWorkflow(sqlite, FIXED_NOW)
    ...
    let releaseFirstProvider: (workbook: {
      readonly id: string
      readonly webViewLink: string
    }) => void = () => undefined
    let providerCalls = 0
    const copyFile = vi.fn(
      async (): Promise<{ readonly id: string; readonly webViewLink: string }> => {
        ...
        return {
          ...
          webViewLink: "https://drive.test/replacement-workbook",
        }
      }
    )
    const listFiles = vi.fn(async () => ({ files: [] }))
  ...
  it("recovers a failed provider attempt and stops after three bounded retries", async () => {
    const sqlite = new Database(":memory:")
    createSchema(sqlite)
    seedDraft(sqlite, FIXED_NOW)
    seedNuTechWorkflow(sqlite, FIXED_NOW)
    ...
    await expect(generateNuTechAirliteWorkbook("project-1")).resolves.toEqual({
      success: false,
      error: "The Airlite workbook generation retry limit has been reached. Try again later.",
    })
    expect(copyFile).toHaveBeenCalledTimes(3)
    const stored = sqlite
      .prepare(
        "SELECT airlite_workbook_status, airlite_workbook_claim_attempt, airlite_workbook_claim_token FROM nutech_order_workflows WHERE id = ?"
      )
      .get("workflow-1") as {
      airlite_workbook_status: string
      airlite_workbook_claim_attempt: number
      airlite_workbook_claim_token: string | null
    }
    expect(stored).toEqual({
      airlite_workbook_status: "stale",
      airlite_workbook_claim_attempt: 3,
      airlite_workbook_claim_token: null,
    })
    copyFile.mockResolvedValue({
      id: "window-reset-workbook",
      webViewLink: "https://drive.test/window-reset-workbook",
    })
