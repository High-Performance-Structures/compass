...
vi.mock("@/lib/work-calendar", () => ({
  projectTodoHref: () => "#todo",
  scheduleItemHref: () => "#schedule-item",
}))
...
vi.mock("@/app/actions/schedule-confirmations", () => ({
  getScheduleTaskChangeProposal: vi.fn(async () => null),
  rejectScheduleTaskChangeProposal: vi.fn(),
  sendScheduleTaskReminder: vi.fn(),
}))

vi.mock("@/app/actions/project-operations", () => ({
  getScheduleTaskTodos: vi.fn(async () => []),
}))
...
vi.mock("@/components/projects/project-task-create-button", () => ({
  ProjectTaskCreateButton: () => null,
}))
