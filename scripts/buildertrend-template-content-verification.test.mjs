...
test("generates one read-only preflight and postflight statement with exact scope", async () => {
  ...
  for (const phase of ["preflight", "postflight"]) {
    ...
    assert.equal(build.templateCount, 18)
    assert.equal(build.contentItemCount, 523)
    assert.equal(build.predecessorCount, 58)
    assert.equal(build.reusableScheduleItemCount, 76)
    assert.equal(build.reusableDependencyCount, 58)
    assert.deepEqual(
