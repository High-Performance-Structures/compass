...
import { customers, organizations, projects, users, vendors } from "./schema"
...
/** Add-only proposals are never retried after an uncertain Sage API attempt. */
export const sageContactCreateProposals = sqliteTable(
  "sage_contact_create_proposals",
  {
    id: text("id").primaryKey(),
    organizationId: text("organization_id").notNull().references(() => organizations.id, { onDelete: "restrict" }),
    kind: text("kind").notNull(),
    ...
    parentSageRecordId: text("parent_sage_record_id").notNull(),
    fieldsJson: text("fields_json").notNull(),
  ...
  (table) => [
    uniqueIndex("sage_contact_create_active_dedupe_unique")
      .on(table.organizationId, table.dedupeKey)
      .where(sql`${table.status} IN ('pending', 'approved', 'running', 'needs_reconciliation')`),
    index("sage_contact_create_claim_idx").on(table.status, table.claimedAt, table.requestedAt),
    index("sage_contact_create_company_idx")
      .on(table.organizationId, table.kind, table.customerId, table.vendorId, table.status),
  ]
)

export const sageContactCreateEvents = sqliteTable(
  "sage_contact_create_events",
  {
    id: text("id").primaryKey(),
    proposalId: text("proposal_id").notNull().references(() => sageContactCreateProposals.id, { onDelete: "restrict" }),
    organizationId: text("organization_id").notNull().references(() => organizations.id, { onDelete: "restrict" }),
    actorUserId: text("actor_user_id").references(() => users.id, { onDelete: "set null" }),
    eventType: text("event_type").notNull(),
    detailJson: text("detail_json").notNull().default("{}"),
  ...
  (table) => [index("sage_contact_create_events_proposal_idx").on(table.proposalId, table.createdAt)]
)
