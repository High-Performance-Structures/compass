...
const { publishOwnerProjectUpdate, updateOwnerProjectUpdateDraft } = await import(
  "@/app/actions/project-field"
)
...
function seedDraft(sqlite: Sqlite): void {
  ...
  run(sqlite, "INSERT INTO projects (id, organization_id) VALUES (?, ?)", "project-1", "org-1")
  run(
    sqlite,
    `INSERT INTO owner_project_updates
       (id, project_id, title, update_date, summary, status, channel,
