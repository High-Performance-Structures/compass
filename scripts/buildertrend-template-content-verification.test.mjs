...
test("generates one read-only preflight and postflight statement with exact scope", async () => {
  ...
  for (const phase of ["preflight", "postflight"]) {
    ...
    assert.equal(build.templateCount, 15)
    assert.equal(build.contentItemCount, 486)
    assert.equal(build.predecessorCount, 50)
    assert.equal(build.reusableScheduleItemCount, 65)
    assert.equal(build.reusableDependencyCount, 50)
    assert.deepEqual(
