...
test("generates one read-only preflight and postflight statement with exact scope", async () => {
  ...
  for (const phase of ["preflight", "postflight"]) {
    ...
    assert.equal(build.templateCount, 7)
    assert.equal(build.contentItemCount, 317)
    assert.equal(build.predecessorCount, 33)
    assert.equal(build.reusableScheduleItemCount, 40)
    assert.equal(build.reusableDependencyCount, 33)
    assert.deepEqual(
