...
test("generates one read-only preflight and postflight statement with exact scope", async () => {
  ...
  for (const phase of ["preflight", "postflight"]) {
    ...
    assert.equal(build.templateCount, 16)
    assert.equal(build.contentItemCount, 499)
    assert.equal(build.predecessorCount, 52)
    assert.equal(build.reusableScheduleItemCount, 68)
    assert.equal(build.reusableDependencyCount, 52)
    assert.deepEqual(
